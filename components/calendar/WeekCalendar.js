...
export default function WeekCalendar({ defaultDate, setDate, setWeekStartDate, weekSelectedDate, tasks }) {
  ...
  const updateWeekDates = (currentDate) => {
    ...
    for (let weekOffset = -2; weekOffset <= 2; weekOffset++) {
      ...
      weekStartDate.setDate(weekStartDate.getDate() - weekStartDate.getDay() + 1 + weekOffset * 7);
      const week = [];
      for (let i = 0; i < 7; i++) {
        ...
        week.push({
          date: day,
          dayOfWeek: days[i],
          selected: day.toDateString() === new Date(weekSelectedDate).toDateString(),
          hasTask: hasTask,
  ...
  const handleScroll = (event) => {
    ...
    if (newWeekIndex !== currentWeekIndex) {
      ...
      newDate.setDate(defaultDate.getDate() + (newWeekIndex - currentWeekIndex) * 7);
        ...
        <View key={weekIndex} style={styles.weekContainer}>
          {week.map((day, index) => (
            ...
            >
              ...
              <Text style={styles.date(day.selected)}>{day.date.getDate()}</Text>
