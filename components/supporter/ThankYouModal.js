...
export default function ThankYouModal({ visible, setVisible, thankYouNotifications = [], setThankYouNotifications }) {
  const [notificationsToShow, setNotificationsToShow] = React.useState(thankYouNotifications);
  ...
  const handleCardClick = async () => {
    ...
    if (notificationsToShow.length > 1) {
      const filteredNotifications = notificationsToShow.filter(item => item.notificationId !== notification.notificationId);
